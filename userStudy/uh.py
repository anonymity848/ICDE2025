 ...
 */
 """
async def update_ext_vec(P: List[Point], C_idx: List[int], u: Point, s, hset: HyperplaneSet, current_best_idx, last_best,
                   frame: List[Point], cmp_option, num_question, dataset_name, epsilon, websocket, session_id):
    ...
    pset_org = PointSet()
    choice = await pset_org.question_for_interaction(websocket, P[C_idx[S[0]]], P[C_idx[S[1]]], num_question)
    if cmp_option == constant.SIMPlEX:
        if choice == 1:
            pset_org.printMiddleSelection(num_question, "UH-Simplex", P[C_idx[S[0]]], P[C_idx[S[1]]], 1, session_id)
            max_idx = S[0]
        else:
            pset_org.printMiddleSelection(num_question, "UH-Simplex", P[C_idx[S[0]]], P[C_idx[S[1]]], 2, session_id)
            max_idx = S[1]

    else:
        if choice == 1:
            pset_org.printMiddleSelection(num_question, "UH-Random", P[C_idx[S[0]]], P[C_idx[S[1]]], 1, session_id)
            max_idx = S[0]
        else:
            pset_org.printMiddleSelection(num_question, "UH-Random", P[C_idx[S[0]]], P[C_idx[S[1]]], 2, session_id)
            max_idx = S[1]
...
async def max_utility(pset: PointSet, u: Point, s, epsilon, maxRound, cmp_option, stop_option, dataset, websocket, session_id):
    ...
    while len(C_idx) > 1 and (rr > epsilon and not others.isZero(rr - epsilon)) and num_question < maxRound:
        num_question += 1
        C_idx.sort()
        C_idx, last_best, current_best_idx = await update_ext_vec(pset.points, C_idx, u, s, hset, current_best_idx,
                                                            last_best, frame, cmp_option, num_question, dataset, epsilon, websocket, session_id)
        if len(C_idx) == 1:
            break
