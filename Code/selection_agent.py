...
parser.add_argument('-reference_tree_ins_strategy', help='default insert strategy for reference tree',
                    default="INS_AREA")
parser.add_argument('-reference_tree_spl_strategy', help='default split strategy for reference tree',
                    default='SPL_MIN_OVERLAP')
...
parser.add_argument('-target_update', type=int, help='update the parameters for target network every ? steps',
                    default=30)
...
parser.add_argument('-network', choices=['strategy', 'spl_loc', 'spl_loc_short', 'sort_spl_loc'],
                    help='which network is used for training', default='sort_spl_loc')
parser.add_argument('-teacher_forcing', type=float,
                    help='the percentage of splits that are with teacher forcing technique', default=0.1)
parser.add_argument('-data_distribution', choices=['uniform', 'skew', 'gaussian', 'china', 'india'],
                    help='data set distribution', default='gaussian')
...
class SplitLearner:
    ...
    def ComputeDenseRewardForList(self, obj_list):
        ...
        for obj in obj_list:
            for i in range(3):
                query = self.tree.UniformDenseRandomQuery(self.config.reward_query_width,
                                                          self.config.reward_query_height, obj)
                # print(query)
    ...
    def ComputeDenseReward(self, object_boundary):
        access_rate_avg = 0
        for i in range(self.config.query_for_reward):
            query = self.tree.UniformDenseRandomQuery(self.config.reward_query_width, self.config.reward_query_height,
                                                      object_boundary)
            access_rate_avg += self.reference_tree.AccessRate(query) - self.tree.AccessRate(query)
        return access_rate_avg / self.config.query_for_reward
    ...
    def Test3(self):
        self.network.load_state_dict(torch.load(
            str(self.config.data_distribution) + '_' + 'k' + str(self.config.action_space) + '_testing' + '.mdl'))
        # self.network.eval()
        # self.ResetObjLoader()
        self.tree.Clear()
        ...
        testing_query_area = 2.0 / 100 * ((self.data_set_bounds[1] - self.data_set_bounds[0]) * (
                    self.data_set_bounds[3] - self.data_set_bounds[2]))
        ...
        while k < 1000:
            ...
            if x - side > self.data_set_bounds[0] and y - side > self.data_set_bounds[2] and x + side < \
                    self.data_set_bounds[1] and y + side < self.data_set_bounds[3]:
        ...
        testing_query_area = 1.0 / 100 * ((self.data_set_bounds[1] - self.data_set_bounds[0]) * (
                    self.data_set_bounds[3] - self.data_set_bounds[2]))
        side = (testing_query_area ** 0.5) / 2
        k = 0
        while k < 1000:
        ...
        testing_query_area = 0.5 / 100 * ((self.data_set_bounds[1] - self.data_set_bounds[0]) * (
                    self.data_set_bounds[3] - self.data_set_bounds[2]))
        side = (testing_query_area ** 0.5) / 2
        k = 0
        while k < 1000:
        ...
        testing_query_area = 0.1 / 100 * ((self.data_set_bounds[1] - self.data_set_bounds[0]) * (
                    self.data_set_bounds[3] - self.data_set_bounds[2]))
        side = (testing_query_area ** 0.5) / 2
        k = 0
        while k < 1000:
        ...
        testing_query_area = 0.05 / 100 * ((self.data_set_bounds[1] - self.data_set_bounds[0]) * (
                    self.data_set_bounds[3] - self.data_set_bounds[2]))
        side = (testing_query_area ** 0.5) / 2
        k = 0
        while k < 1000:
        ...
        testing_query_area = 0.01 / 100 * ((self.data_set_bounds[1] - self.data_set_bounds[0]) * (
                    self.data_set_bounds[3] - self.data_set_bounds[2]))
        side = (testing_query_area ** 0.5) / 2
        k = 0
        while k < 1000:
        ...
        testing_query_area = 0.005 / 100 * ((self.data_set_bounds[1] - self.data_set_bounds[0]) * (
                    self.data_set_bounds[3] - self.data_set_bounds[2]))
        side = (testing_query_area ** 0.5) / 2
        k = 0
        while k < 1000:
    ...
    def Test2(self):
        self.network.load_state_dict(
            torch.load(str(self.config.data_distribution) + '_' + 'k' + str(self.config.action_space) + '.mdl'))
        # self.network.eval()
        # self.ResetObjLoader()
        self.tree.Clear()
        self.reference_tree.Clear()  # CONTINUE FROM HERE
    ...
    def Train5(self):
        ...
        for epoch in range(self.config.epoch):
            ...
            for part in range(self.config.parts):
                ...
                for training_id in range(objects_for_train):
                    print(["Epoch", epoch, "Parts", part, "Training obj ID", training_id, "Total training obj",
                           objects_for_train])
                    # line = fin.readline()
                    ...
                    if e % 500 == 0:
                        ...
                        if epoch > 0:
                            self.config.epsilon = max(self.config.epsilon * self.config.epsilon_decay,
        ...
        torch.save(self.network.state_dict(),
                   str(self.config.data_distribution) + '_' + 'k' + str(self.config.action_space) + '.mdl')
        # reward_log.close()
        # loss_log.close()
